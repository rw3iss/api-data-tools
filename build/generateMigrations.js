...
var require_dist = __commonJS((exports2, module2) => {
  ...
  var camelCase = _interopDefault(require_lodash2());
  ...
  var Definitions = class extends Array {
    ...
    get(arg) {
      if (isOption(arg)) {
        return re.short.test(arg) ? this.find((def) => def.alias === getOptionName(arg)) : this.find((def) => def.name === getOptionName(arg));
      } else {
    ...
    for (var resourceName in newSchema2) {
      if (newSchema2.hasOwnProperty(resourceName)) {
        if (!currentSchema.hasOwnProperty(resourceName)) {
          if (typeof newSchema2[resourceName].persistent == "undefined" || newSchema2[resourceName].persistent) {
            ops.up.push({type: "create_table", name: resourceName, data: newSchema2[resourceName]});
...
// src/scripts/generateMigrations.ts
...
function _readFile(path3) {
  return new Promise((resolve5, reject) => {
    import_fs5.readFile(path3, "utf8", (err, data) => {
      if (err) {
        console.log("Error reading file", path3);
        return reject(err);
      } else {
        return resolve5(data ? JSON.parse(data) : "");
      }
    });
  });
}
